...
- Local daemon bridge: 扩展 ↔ native host (`ai.wiseria.pie`) ↔ daemon IPC（mac unix socket / Windows named pipe，接口不依赖哪一种）；wire 类型唯一权威源 `src/types/local-bridge.ts`（daemon 相对 import，加法演进，PROTOCOL_VERSION=2）。**协议只传语义**（id / kind / 能力 / 用户内容），不传系统怎么做（绝对路径、pid、平台 URL、win32 布尔）——ADR 0012；新代码不得把存量违规字段当合同。桥**意外**断开在 SW 存活期内自动退避重连（1s→30s 封顶，重连尝试自身失败也续排梯子；用户关「本地打通」开关则不重连），SW 唤醒由 `initBridgeAndMigrate()` 兜底（先桥后幂等 IDB→盘迁移的时序不可倒）。handoff 候选表（`daemon/src/agents.ts`）= 唯一 launch 权威（wire 只传 id）：Claude/Codex/Cursor 各 app+terminal 两形态 + OpenCode/Pi terminal + DeepSeek Harness（mac：`dsh-app` Web UI + `dsh-terminal` 仅 headless，2026-08-22 真机过已 `verified:true` 进默认可用集；Windows 表同样两条，走 `%APPDATA%\npm\dsh.cmd`，app 类无安装包靠 bin 回落检出），mac 10 条；**改表前按平台分别调研安装落点**（mac bundle / Windows Uninstall 或 Store MSIX 不是同一回事，不可互推）。检测模块解耦：`detect-darwin.ts` 与 `detect-win32.ts` 互不调用，只有 `detectAgents` 分发（ADR 0011）。命令必须在**该平台**真机验证过才进默认可用集。**App 形态必须交付「打开 + 预填文件夹 + 预填提示词」**（ADR 0013）：优先官方一条深链；官方把目录和 prompt 拆开则按序走两条官方通道。没有这条路径就不要加 App 行（DeepSeek Harness 是唯一例外：无官方深链，走本机 loopback Web UI 编排）。mac detect 问用户 login shell 要 PATH（daemon 跑在 launchd 裸 PATH 下，否则一个 CLI 都看不见）并解出绝对路径，`start.command` 里 exec 绝对路径
